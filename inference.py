...
from ribofold.data.balstn import BLASTN
from ribofold.ribofold import ribofold
from ribofold.config import ribofold_config
from ribofold.utils import get_device, save_ss2ct, timing
from ribofold.relax.relax import AmberRelaxation
from ribofold.utils.alphabet import get_features

@torch.no_grad()
def main(config):
    '''
    ribofold Inference pipeline
    '''

    os.makedirs(config.output_dir, exist_ok=True)

    logger = logging.getLogger('ribofold Inference')
    ...
    logger.info(f'Constructing ribofold')
    model = ribofold(ribofold_config)
    ...
    with timing('ribofold Inference', logger=logger):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--binary_dpath", help="Path to the pretrained model, default ./ribofold/data/bin", default='./ribofold/data/bin')
